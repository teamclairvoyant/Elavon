...
class UniquenessCheck:
    ...
    def execute(self):
        ...
        failed_records_query = f"select {base_criteria_column} from {entity_physical_name} where {filter_condition} {base_criteria}"
        total_records_query = f"select count(1) as total_count from {entity_physical_name}"
