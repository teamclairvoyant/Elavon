...
class LengthCheck:
    ...
    def execute(self):
        ...
        column_value_length = self.context.get_rule_property('COLUMN_VALUE_LENGTH')
        ...
        base_criteria = base_criteria.replace('{COLUMN_VALUE_LENGTH}', column_value_length)
        failed_records_query = f"select {primary_key} from {entity_physical_name} where {base_criteria} and {filter_condition}"
        total_records_query = f"select count(1) as total_count from {entity_physical_name} where {filter_condition}"
